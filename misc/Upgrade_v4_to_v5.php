...
class Upgrade_v4_to_v5 extends MX_Controller {
    ...
    function update_timezone() {
        ...
        $data = array (
        	"smtp_status" => $this->common->get_field_name('value','system',array("name" => "smtp")),
        	"smtp_host" => $this->common->get_field_name('value','system',array("name" => "smtp_host")),
        	"smtp_port" => $this->common->get_field_name('value','system',array("name" => "smtp_port")),
        	"smtp_pass" => $this->common->get_field_name('value','system',array("name" => "smtp_pass")),
        	"smtp_user" => $this->common->get_field_name('value','system',array("name" => "smtp_user")),
        );
        $data['smtp_pass'] = $this->common->encode($data['smtp_pass']);
        $this->db->where("id",1);
        $result = $this->db->update('invoice_conf',$data);
        if($result == 1){
        	$names = array("smtp_host","smtp_port","smtp_pass", "smtp_user");
        ...
        $this->db->query("UPDATE `sip_devices` set `is_pbx` = 0");
        $this->index();
    }
    function index() {
        $sip_devices = $this->db->get('sip_devices');
        if($sip_devices->num_rows > 0){
               foreach ($sip_devices->result_array() as $row) {
                $params = json_decode ( $row ['dir_params'], true );
                if(isset($params['password']) && $params['password'] != ''){
                    $params['password']=$this->encode($params['password']);
                    $params_enc=json_encode($params);
                    $data=array("dir_params"=>$params_enc);
        ...
        if($gateways->num_rows > 0){
               foreach ($gateways->result_array() as $value) {
                $gateway_data = json_decode ( $value ['gateway_data'], true );
                if(isset($gateway_data['password']) && $gateway_data['password'] != ''){
                    $gateway_data['password'] = $this->encode($gateway_data['password']);
                    $params_enc=json_encode($gateway_data);
                    $data=array("gateway_data"=>$params_enc);
