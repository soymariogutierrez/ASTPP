...
DROP TABLE IF EXISTS temp_sip_devices;

CREATE TABLE temp_sip_devices (
...
ON sip_devices FOR EACH ROW BEGIN
    IF NEW.id IS NOT NULL THEN
        INSERT INTO temp_sip_devices(sip_device_id,action_id,action_date) VALUES(new.id,1,NOW()); 
    END IF;
END$$
...
BEGIN
	INSERT INTO temp_sip_devices (sip_device_id,action_id,action_date) VALUES (old.id,2,NOW()) ON DUPLICATE KEY UPDATE action_id = 2;
END$$
...
ON sip_devices FOR EACH ROW
BEGIN
	INSERT INTO temp_sip_devices (sip_device_id,action_id,action_date) VALUES (old.id,3,NOW()) ON DUPLICATE KEY UPDATE action_id = 3;
END$$
...
BEGIN
    IF NEW.status <> OLD.status THEN 
        INSERT INTO temp_sip_devices (sip_device_id,action_id,action_date) SELECT id,2,NOW() FROM sip_devices WHERE accountid=old.id;
    END IF;
END$$
