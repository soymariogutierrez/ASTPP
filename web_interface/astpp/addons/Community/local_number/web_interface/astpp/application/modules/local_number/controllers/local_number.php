...
/*	function local_number_import() {
			...
			if (isset ( $csv_data ['number'] ) && $csv_data ['number'] != '' && $i != 0) {
				...
				if ($str != "") {
					...
					if (! in_array ( $csv_data ['number'], $number_arr )) {
						...
						if ($number_count > 0) {
							$invalid_array [$i] = $csv_data;
							$invalid_array [$i] ['error'] = gettext('Duplicate Local number found from database');
						} else {
							...
							if($csv_data ['country_id'] != '')
							{
								...
								$invalid_array [$i] = $csv_data;
								$invalid_array [$i] ['error'] = gettext('Country not valid.');
							}
							
						}
					} else {
						$invalid_array [$i] = $csv_data;
						$invalid_array [$i] ['error'] = gettext('Duplicate Local number found from import file.');
					}
				}
		...
		if ($add_array ['id'] != '') {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				echo json_encode ( array (
						"SUCCESS" => $add_array ["number"] .' '.gettext("Local number updated successfully!") 
				) );
				exit ();
			}
		} else {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				echo json_encode ( array (
						"SUCCESS" => $add_array ['number'] .' '.gettext("Local number added successfully!") 
				) );
				exit ();
			}
		}
	...
	function local_number_remove($id) {
		...
		$this->session->set_flashdata ( 'astpp_notification', gettext('Local number removed successfully!'));
		redirect ( base_url () . 'local_number/local_number_list/' );
	}
	function local_number_delete_multiple() {
		 // Kinjal Issue Number 2257 Addons - Roles and Permission management for customer account
		  ...
		  if($account_data['type'] == 0){
			$this->common->validate_module_access_level('local_number','delete','local_number_list_customer',"local_number/local_number_list_customer/");
		  }
		//   END
		$add_array = $this->input->post ();
		...
		$where = 'IN (' . $add_array ['selected_ids'] . ')';
		$this->db->where ( 'local_number_id ' . $where );
	...
	function local_number_action($type,$edit_id) {
		...
		if($type == 'add') {
			...
			// Kinjal Issue 1452 Local number - Customer/Admin is able to add same local number multiple times
			$query =   $this->db_model->getSelect("*", "local_number_destination",array("account_id" => $edit_id,"local_number_id"=>$_POST['local_number_id']) );
				if( $query->num_rows() > 0){
					$this->session->set_flashdata ( 'astpp_notification', gettext('This local number already added, Please select different local number') );
		...
		$this->session->set_flashdata ( 'astpp_errormsg', gettext('Local number forwarding successfully!') );
		redirect ( base_url () . 'accounts/customer_local_number_forwarding/'.$edit_id.'/' );
	}
...
/* harsh s for remove speed dial along with local number destination */
	...
	function local_number_list_customer() {
		// Kinjal Issue Number 2257 Addons - Roles and Permission management for customer account
		 ...
		 if($accountinfo['type'] == 0){
			 $this->common->validate_module_access_level('local_number','list','local_number_list_customer',"");
		 }
		 // END
		$data ['username']    = $this->session->userdata ( 'user_name' );
		$data ['page_title']  = gettext ( 'Local Number' );
		...
		//$reseller_id=($accountinfo ['type'] == 1 || $accountinfo ['type'] == 5) ? $accountinfo ['id'] : 0;
			...
			}elseif($accountinfo ['type'] == 0 || $accountinfo ['type'] == 3){
	...
	function customer_local_number_forwarding_json($edit_id='',$accounttype='customer') {
		...
		$json_data      = array ();
		$instant_search = $this->session->userdata ( 'left_panel_search_' . $accounttype . '_local_number' );
		$account_arr    = ( array ) $this->db->get_where ( 'accounts', array ("id" => $edit_id ) )->first_row ();
		//Rupa issue 1636 Local Number - Searching with local number not working in customer edit page
		$like_str       = ! empty ( $instant_search ) ? "(destination_name like '%$instant_search%'
					OR  local_number.number like '%$instant_search%' OR destination_number like '%$instant_search%')" : null;
		if (! empty ( $like_str ))
			$this->db->where ( $like_str );
		$where = array (
				"account_id" => $edit_id 
		...
		$this->db->where($where);
		// END
		$this->db->select('local_number.number as number,local_number_destination.local_number_id as local_number_id,local_number_destination.destination_name,local_number_destination.destination_number,');
		if (! empty ( $like_str ))
			$this->db->where ( $like_str );
		$this->db->from('local_number');
		$this->db->join('local_number_destination', 'local_number.id = local_number_destination.local_number_id'); 
		$query = $this->db->get();
	...
	function local_number_customer_save() {
		...
		if ($add_array ['id'] != '') {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				echo json_encode ( array (
						"SUCCESS" => $add_array ["name"] . gettext("Local number added successfully!")
				) );
				exit ();
			}
		}
	...
	function local_number_destination_customer_save() {
        ...
        if ($accountinfo['type'] == 0) {
            if($add_array['id'] == ''){
                $this->common->validate_module_access_level('local_number','create','local_number_list_customer',"local_number/local_number_list_customer/");
            }else{
                $this->common->validate_module_access_level('local_number','edit','local_number_list_customer',"local_number/local_number_list_customer/");
            }       
        }
        // END     
		$data ['form'] = $this->form->build_form ( $this->local_number_form->get_local_number_customer_form_field ($add_array['id']), $add_array );
		...
		if ($add_array ['id'] != '') {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				echo json_encode ( array (
						"SUCCESS" => gettext("Local number updated successfully!")
				) );
				exit ();
			}
		} else {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				$this->local_number_model->add_local_number_customer ( $add_array );
				echo json_encode ( array (
						"SUCCESS" => gettext("Local number added successfully!")
				) );
				exit ();
			}
		}
	...
	function local_number_customer_action($type,$edit_id){
		  ...
		  $accountinfo = $this->session->userdata ( 'accountinfo' );
		  if ($accountinfo['type'] == 0) {
			  $this->common->validate_module_access_level('local_number','add','local_number_list_customer',"local_number/local_number_list_customer/");
		  }
		  // END
