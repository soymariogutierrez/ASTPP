...
//14_09_harsh
					...
					$message = str_replace("#NAME#", $this->common->get_field_name_coma_new('first_name,last_name,number,company_name','accounts',$account_data['id']),$message);
					...
					$mail_details_array= array(
								...
								'attachment'=>'',
								'status'=>1,
								//Manish Issue 1703 - Added below three fileds
								'emailstatus'=>1,
								'smsstatus'=>1,
								'alertstatus'=>1,
								//END
...
//print_r($account_data); exit;
				...
				if(!empty($account_data)){
					...
					$add_array=array(
							//Manish issue 1783 (Support Ticket - "Customer-Reply" status set when new ticket open by customer of reseller using mail) - changes ticket_type status to 0 from 2.
							'ticket_type'=>0,
							//END
							'priority'=>1,
					...
					if($email_receiver !=''){
						...
						$subject = str_replace("#TICKET_SUBJECT#", $email_subject,$subject);
//print_r($to_email_address_array);
						//Manish Issue 1703 - Removed HTML Tags from email
						$message = strip_tags($message);
						//END
						foreach($to_email_address_array as $receiver_emailid){
							$mail_details_array= array(
...
//echo '------------------------'; exit;
						...
						$auto_message = str_replace("#REPLY_TYPE#", $this->_get_ticket_type_message('','',0), $auto_message);

						$auto_message = str_replace("#NAME#", $this->common->get_field_name_coma_new('first_name,last_name,number,company_name','accounts',$account_data['id']),$auto_message);
