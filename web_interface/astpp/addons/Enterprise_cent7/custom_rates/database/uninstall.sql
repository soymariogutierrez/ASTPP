...
delete from `menu_modules` where `module_name`='custom' AND `module_url`='personalized_rates/personalized_rates_list/';

delete from `roles_and_permission` where `module_name`='custom_rates' AND `module_url`='personalized_rates_list' AND `login_type`=0;

delete from `roles_and_permission` where `module_name`='custom_rates' AND `module_url`='personalized_rates_list' AND `login_type`=1;
